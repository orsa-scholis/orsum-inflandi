...
func (s *Server) Start() {
	logger.Info("legacyServer started listening")
	...
	for {
		...
		if nil != err {
			select {
			case <-s.stoppedChan:
				logger.Info("legacyServer is stopped, can't accept anymore connections")
				return
			default:
...
func (s *Server) joinGame(gameID uuid.UUID, user *Client) (returnState ClientState) {
	returnState = InLobby
...
func (s *Server) broadcastMessage(m UserMessage, me *Client) {
